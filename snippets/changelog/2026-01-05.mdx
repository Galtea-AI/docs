...
Test cases now accept a new property: `expected_tools`. This allows you to define exactly which tools an agent should invoke during a specific test scenario. [Read more about Test Case structure](/sdk/api/test-cases#properties).
...
This metric automatically compares the `tools_used` against the `expected_tools` to determine if the agent selected the right functions to solve the user's problem. This is critical for ensuring reliability in agentic workflows. [See metric details](/concepts/metrics#tool-correctness).
