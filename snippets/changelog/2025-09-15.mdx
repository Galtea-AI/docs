...
- **Selectable Conversation Styles**: When creating a [Scenario Based Test](/concepts/product/test/scenario-tests), you can now choose a conversation style (`written` or `spoken`). This influences the tone and formality of the simulated user's dialogue, enabling more realistic testing. This is available under the `strategies` parameter in the [test creation method](/sdk/api/test/create).
