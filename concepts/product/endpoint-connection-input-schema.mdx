...
## How It Works with Dataset Generation

When you generate datasets (accuracy, behavior, or security) for a product that has an Input Schema:
...
<Note>
  An attached file is not a schema field. The download URL of an attachment is never part of `input`, so name it with [`input_files`](/concepts/product/endpoint-connection-template-syntax#attached-files-and-audio) instead.
</Note>
...
<CardGroup cols={2}>
  <Card title="Template Syntax" icon="code" href="/concepts/product/endpoint-connection-template-syntax">
    Full placeholder reference: `{{ input.field_name }}`, `{{ context.field_name }}`, `past_turns`, `input_files`, and more.
  </Card>
  ...
  <Card title="Test Case Inputs" icon="list" href="/concepts/product/dataset/test-case">
  ...
  <Card title="Behavior Datasets" icon="user-group" href="/concepts/product/dataset/behavior-datasets">
    Multi-turn behavior datasets with scenarios and personas.
  </Card>
</CardGroup>
