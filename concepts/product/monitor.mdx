...
In the dashboard, open a product and go to its **Settings** tab, then the **Monitors** card. That card is the default home for monitor management and lists every monitor for the product.

Power users can turn on **Show all tabs** (in User Settings > Advanced) to also get a dedicated **Monitors** tab on the product page. It appears only in **Production** view mode, because monitors watch production sessions; use the view-mode selector at the top of the product page to switch to Production if you do not see it.
...
A monitor's own page shows the results of that one monitor. To see the trend across every production evaluation of the product, open the product's **Analytics** tab in **Production** view mode. It shows an **evaluation scores over time** chart: the average score per day across all production evaluations that match your current filters.

The chart is only in Production view mode, because it is a production-monitoring view. Use the toggle on the card to switch between two views: **by metric** (one line per metric) and **by specification** (one line per specification, built from the metrics linked to each specification). It follows the same filters as the rest of the page, so changing the version, test, metric, language, or time frame updates it too. It aggregates every matching production evaluation, including ones you triggered by hand, so it is a product-wide trend, not one monitor's view.
