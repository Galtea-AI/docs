...
<Info>
    In the [SDK](/sdk/api/test/create), you can select one or more security strategies to apply to your test cases. These strategies are applied to the selected threat, generating variations of the threat prompt with each chosen strategy. Note that each Security test supports exactly **one threat** — to test multiple threats, create a separate test for each.
</Info>
...
- **Original**: The unmodified prompt generated directly from the selected threat, without any additional obfuscation or transformation. This serves as the baseline for comparison in Security & Safety tests and is always included by default (it cannot be deselected).
...
When you generate Security & Safety tests in Galtea, you can select one or more strategies. Each selected strategy is applied to the chosen threat, creating multiple variations of the threat prompt. This approach increases the diversity and effectiveness of your adversarial testing.
...
When creating a Security & Safety test in the Galtea platform:
- Choose one threat (e.g., misuse, data_leakage, etc.). Each test supports exactly one threat.
