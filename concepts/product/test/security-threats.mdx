...
<Info>
  **SDK Parameter Note:** In the Galtea SDK, "Threats" are specified using the `variants` parameter in the [`galtea.tests.create()`](/sdk/api/test/create) method when creating `SECURITY` tests. **Only one threat can be specified per test.** For example: `galtea.tests.create(..., type="SECURITY", variants=["toxicity"])`. To test multiple threats, create a separate test for each.
</Info>
