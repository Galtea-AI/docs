...
Behavior tests in Galtea are designed to evaluate the conversational capabilities of your [product](/concepts/product) through multi-turn dialogue interactions. These tests use the [Conversation Simulator](/concepts/product/test/case/conversation-simulator) to create realistic conversations with synthetic users that have specific goals, personalities, and behaviors.

Unlike [Accuracy Tests](/concepts/product/test/quality-tests) that focus on single-turn question-answer pairs, or [Security & Safety Tests](/concepts/product/test/red-teaming-tests) that test security vulnerabilities, Behavior tests evaluate how well your AI product can:
