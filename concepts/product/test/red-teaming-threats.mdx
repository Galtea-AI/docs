...
<Info>
  **SDK Parameter Note:** In the Galtea SDK, "Threats" are specified using the `variants` parameter in the [`galtea.tests.create()`](/sdk/api/test/create) method when creating `SECURITY` tests. For example: `galtea.tests.create(..., type="SECURITY", variants=["Toxicity", "Misuse", "data_leakage"])`.
</Info>
