...
<Note>
  Already have a test and want more cases? If Galtea generated the test, use [Extend](/concepts/product/test/extend-test-cases) to add more cases the same way it was created. For any test with cases, use [Data Augmentation](/concepts/product/test/data-augmentation) to expand it from its existing examples.
</Note>
...
<ResponseField name="Status" type="Enum">
  ...
  - `EXTENDING`: Test cases are being extended by re-running the test's original generator
  - `CANCELLED`: Test generation was cancelled
</ResponseField>
...
<ResponseField name="Background Noise Profile" type="Enum">
  Background noise mixed into the simulated caller's audio for voice tests run over a [phone connection](/concepts/product/phone-connection). Only applies to voice tests; has no effect otherwise.
  Possible values:
