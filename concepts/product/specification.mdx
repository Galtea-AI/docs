...
<Note>
  In the dashboard, specifications are managed in two places, one per **scope**. Both are sidebar entries of the open product: the **Specifications** section holds the positive scope (Capability and Policy), and the **Settings** section holds the negative scope (Inability). Both use the same table and the same creation form; the form knows which scope it is in from the `scope` parameter in its URL (`?scope=positive` or `?scope=negative`).
