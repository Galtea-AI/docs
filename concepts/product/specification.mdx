...
<Note>
  In the dashboard, specifications are managed in two places, one per **scope**. The product's **Specifications** tab holds the positive scope (Capability and Policy). The product's **Settings** tab holds the negative scope (Inability). Both use the same table and the same creation form; the form knows which scope it is in from the `scope` parameter in its URL (`?scope=positive` or `?scope=negative`).
