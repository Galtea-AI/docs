...
## Opening a Run yourself

You can also open a Run yourself, and attribute your own sessions and evaluations to it. Such a Run records work you performed, so it names no launch of the platform.

The steps are:

1. Create the Run with `POST /runs`, with `galtea runs create`, or with [`galtea.runs.start()`](/sdk/api/run/start).
2. Pass the returned id as `runId`, or as `run_id` in the SDK, on each call that creates a session or an evaluation. Four calls accept it: `POST /sessions`, `POST /evaluations/fromSession`, `POST /evaluations/fromInferenceResult` and `POST /evaluations/singleTurn`.
3. Close the Run with `POST /runs/{id}/close`, with `galtea runs close`, or by leaving the SDK `with` block.

<Warning>
  Pass a `runId` only for a Run you opened with `POST /runs`. A Run the platform opened already
  records its own launch, so it refuses work you attribute, and the answer says so.
  `POST /evaluations/fromVersion`, `POST /evaluations/fromSessions` and
  `POST /inferenceResults/generate` refuse a `runId` outright, because each one opens its own Run.
</Warning>
...
- **`RUNNING`** — the work is in progress. Every new Run starts here.
- **`COMPLETED`** — the work finished and created its evaluations.
- **`FAILED`** — the launch produced no evaluation. A launch that fails its endpoint check ends here with zero evaluations, and so does one whose every session failed. The `error` field says why.
- **`CANCELLED`** — you cancelled the Run, or the queue job that carried it out.

A Run the platform opened settles with its own launch. The launch has one outcome, and that outcome becomes the status of the Run.

A Run you opened settles only when you close it, and it always ends as `COMPLETED`. You performed the work yourself, so the platform has no outcome of its own to report.
...
The `kind` of the source decides what "again" means. You do not choose:

- A Run of kind `GENERATE_INFERENCES` is **re-launched**. Galtea drives the version again, creates new sessions, and scores them.
- A Run of any other kind is **re-evaluated**. Galtea scores sessions that already exist again, with the same metrics, and creates no session.
- A Run you performed yourself carries no `kind`. It is re-evaluated too. Galtea reads the Run's own evaluations for the sessions and the metrics to score again. No session is created, because only you can produce one.
...
A Run whose evaluations name no session is refused, because there is nothing to score again. So is one that scored more sessions than a single repetition can cover; score those from the sessions list instead, in batches. The answer names the reason.

## Cancelling a run

`POST /runs/{id}/cancel` stops a Run that is still running, and the work behind it. The sessions and evaluations it already created are kept.

Only a Run the platform queued a job for can be cancelled. It is the one carrying a `jobId`. Every other Run finishes inside the request that started it, so by the time you read it there is nothing left to stop, and a Run you opened yourself carries no job at all.
...
Open a row to reach the run's own page. It has three tabs:

- **Overview** — a **Run** card with the status, the version, the finish time and the counts, a **What it ran** card with the kind, the inputs, the error and the request, and how each metric scored. For a run you opened yourself, the second card says instead that you performed the work.
- **Sessions** — every session the run created or scored.
- **Evaluations** — the evaluations the run launched.

The **What it ran** card carries the actions. **Cancel** is shown while the run is still running and the platform queued a job for it. Otherwise **Run again** is shown, for a run Galtea can repeat. The two are never shown together.

Evaluation tables and session tables both show a **Run** column and a **Runs** filter, so you can narrow either list down to one run. In both, the column links to the run's page. Session tables hide both in the Production view, because production traffic never belongs to a Run.
...
The [REST API](/api-reference) offers `POST /runs`, `POST /runs/{id}/close`, `POST /runs/{id}/relaunch`, `POST /runs/{id}/cancel`, `GET /runs`, `GET /runs/{id}`, `GET /runs/{id}/metric-summary` and `DELETE /runs/{id}`. `GET /evaluations` accepts a `runIds` filter, so you can read every evaluation of a run.
...
The SDK write methods take the same id as `run_id`: [`sessions.create`](/sdk/api/session/create), [`sessions.get_or_create`](/sdk/api/session/get-or-create), [`evaluations.create`](/sdk/api/evaluation/create), [`evaluations.create_single_turn`](/sdk/api/evaluation/create-single-turn), [`evaluations.run`](/sdk/api/evaluation/run) (only when you pass an `agent`, because the queued mode performs its own launch) and [`traces.create_and_evaluate`](/sdk/api/trace/create-and-evaluate). [`evaluations.list`](/sdk/api/evaluation/list) reads them back with the same `run_id` filter, and each `Session` and `Evaluation` carries its own `run_id`.

The [CLI](/cli/usage) mirrors those operations: `galtea runs list`, `galtea runs get`, `galtea runs create`, `galtea runs close`, `galtea runs relaunch`, `galtea runs cancel`, `galtea runs get-metric-summary` and `galtea runs delete`.
...
<ResponseField name="versionId" type="string" optional>
  The ID of the version the run evaluated. It is empty for a metric replay, because a replay can span several versions of one product, so no single version would be correct.
</ResponseField>

<ResponseField name="userId" type="string" optional>
  The ID of the user who started the run. It is empty when the run came from an automation with no signed-in user.
</ResponseField>
...
<ResponseField name="openedBy" type="Enum">
  Who opened the run. `LAUNCH` when a launch endpoint opened it; the run then ends with the launch it holds. `CALLER` when `POST /runs` opened it; only `POST /runs/{id}/close` ends it.
</ResponseField>
...
<ResponseField name="kind" type="Enum" optional>
  What the platform did in this run. It is empty on a run you opened with `POST /runs`. Possible values:
  - `GENERATE_INFERENCES`: the platform generated sessions from test cases, and scored them when the request named a metric. `POST /evaluations/fromVersion`, `POST /evaluations/fromTestCases` and `POST /inferenceResults/generate` reach it.
...
<ResponseField name="jobId" type="string" optional>
  The ID of the queue job that carried out the run. It is empty when the work finished inside the request, with no job. A run carrying one can be cancelled.
</ResponseField>

<ResponseField name="error" type="string" optional>
  Why the run failed. It is empty unless the status is `FAILED`.
</ResponseField>

<ResponseField name="request" type="object" optional>
  The launch request as the platform received it. It is empty on a run you opened with `POST /runs`.
</ResponseField>

<ResponseField name="resolvedInputs" type="object" optional>
  What the request resolved to at dispatch: the test cases, sessions or metrics the platform worked on. Its keys depend on `kind`. It is empty on a run you opened with `POST /runs`.
</ResponseField>
