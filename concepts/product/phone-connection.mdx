...
2. In **Development** view mode, open the **Connections** section in the sidebar (under **Product**), then the **Phone** tab
...
Both sides of every turn are recorded and stored on the `Trace` for that turn:
...
This means the full phone conversation — both the synthetic user side and the agent side — can be played back turn by turn in the dashboard's audio player and read programmatically via the SDK. See [Trace](/concepts/product/version/session/trace) for the envelope shapes.
