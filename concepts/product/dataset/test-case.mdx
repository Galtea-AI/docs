...
**Every metric that reads the input is skipped**, with or without text alongside the file, because evaluators cannot read files yet. Conversation-level metrics skip a session on such a test case too. Score these test cases with a metric that does not read the input, such as a JSON Field Match metric comparing your output field by field. The rule applies to the test case only: a trace you log yourself is scored on whatever text its input carries, and any file or audio part on it is stored as sent and ignored by the judge. See [Troubleshooting skipped evaluations](/concepts/metric/evaluation-parameters#troubleshooting-skipped-evaluations).

Editing a test case that has files follows the usual [revision](/concepts/product/dataset/test-case#revisions) behaviour: a revision may add or replace files, and the superseded revision keeps its own. Send `input` as the whole object, including the files you want to keep. **The edit must leave at least one file.** Plain text is refused, and so is an object with an empty `content`, because either one would turn a document test case into a text one and every metric that skipped it would start scoring it. To get the text-only test case, create it and delete this one.
