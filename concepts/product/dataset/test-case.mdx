...
Read the files back from `test_case.input_files`. To run your own pipeline over an attached file, fetch its bytes with [`galtea.storage.download()`](/sdk/api/storage/download), passing the `InputFile` straight from `test_case.input_files`. [Evaluate Document Inputs](/sdk/tutorials/evaluate-document-inputs) walks that whole round trip, from the CSV to a scored answer.
...
- **Galtea cannot run the inference for you.** Send the document to your own pipeline and upload the output as an inference result.
