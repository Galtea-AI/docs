...
<ResponseField name="Live Turns to Run" type="Number">
  The maximum number of turns the simulator runs, on top of the predefined conversation if the test case has one. This prevents conversations from running indefinitely. In the CSV file and in the SDK the field is named `max_iterations`. **Example**: 10
</ResponseField>
