...
<Info>
  ...
  One limit: a dataset whose test cases carry audio or file inputs cannot be augmented. The engine cannot create a new attachment, so every generated test case would point at the same one.
</Info>
...
You can augment any existing dataset without audio or file inputs directly from the [Galtea dashboard](https://platform.galtea.ai/):

<Steps>
  <Step title="Navigate to your dataset">
    Pick **Datasets** in the sidebar and select the dataset you want to augment. The sidebar keeps **Datasets** behind **More** unless you turn on **Show all tabs** in your settings.
  </Step>
