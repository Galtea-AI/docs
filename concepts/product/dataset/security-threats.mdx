...
<Info>
  **SDK Parameter Note:** In the Galtea SDK, "Threats" are specified using the `variants` parameter in the [`galtea.datasets.create()`](/sdk/api/dataset/create) method when creating `SECURITY` datasets. **Only one threat can be specified per dataset.** For example: `galtea.datasets.create(..., type="SECURITY", variants=["toxicity"])`. To test multiple threats, create a separate dataset for each.
</Info>
...
| **Custom** | Allows the generation of highly specific adversarial test cases that target the unique vulnerabilities and edge cases of your AI product. | - | - |
