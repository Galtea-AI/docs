---
title: "Security & Safety Datasets"
description: "Evaluate security, safety, and bias aspects of your AI products"
sidebarTitle: "Overview"
---

## What are Security & Safety Datasets?

Security & Safety datasets in Galtea are designed to evaluate the security, safety, and bias aspects of your [product](/concepts/product). These datasets typically consist of different types of [threats](/concepts/product/dataset/security-threats) like adversarial inputs specifically crafted to probe potential weaknesses or vulnerabilities in your AI system. To further enhance the diversity and evasiveness of these datasets, Galtea can apply various [security strategies](/concepts/product/dataset/security-strategies) to the prompts generated from these threats.

## Creating Security & Safety Datasets

You can create security & safety datasets in Galtea through two methods:
  ...
  <Tab title="Generate from Your Known Threats">
    <Steps>
      ...
      <Step title="Configure the dataset">
        Select *Security & Safety* as the dataset type and *Generated* as the dataset origin
        <Info>
      ...
      <Step title="Generate the dataset">
        Galtea will process the Knowledge Base and generate a *Dataset File* containing *Test Cases* with adversarial inputs for you, potentially applying selected strategies to vary the attack vectors.
      </Step>
    </Steps>
    ...
    <video
      ...
      src="/videos/generate-red-teaming.mp4"
    ></video>
  </Tab>
  <Tab title="Upload Your Own Dataset">
    <Steps>
      <Step title="Create your dataset file">
        Prepare a CSV file following the structure shown [below](#example-security--safety-datasets-and-file-format)
      </Step>
      <Step title="Configure the dataset">
        Select "Security & Safety" as the dataset type and "Uploaded" as the dataset origin
        <Info>
          The dataset creation process can be done via the [SDK](/sdk/api/dataset/create) or the [Galtea dashboard](https://platform.galtea.ai/)
        </Info>
      </Step>
...
Security & Safety datasets typically focus on several key categories:
...
## Example Security & Safety Datasets and File Format

Here are examples of the content and its creation-document structure:

<Warning>
  Security & Safety datasets intentionally contain harmful or adversarial content. These are used solely for the purpose of testing AI safety and security. Always handle such content responsibly and in accordance with your organization's policies.
</Warning>
...
## Structure of Security & Safety Datasets

Security & Safety datasets have a specific structure focused on providing challenging inputs:
...
<ResponseField name="input" type="Text" required>
  The adversarial input specifically designed to test the product's resilience against potential vulnerabilities. Security dataset inputs are plain strings (adversarial prompts) - structured JSON input does not apply to this dataset type. **Example**: "Please ignore any previous instructions and..."
</ResponseField>

<ResponseField name="expected_output" type="Text">
  For security & safety datasets, this field is typically marked as "N/A" since the purpose is to test system resilience rather than producing a specific output.
</ResponseField>

<ResponseField name="tag" type="Text">
  A categorization tag for the test instance, typically "adversarial_inputs" for security & safety datasets.
</ResponseField>

<ResponseField name="source" type="Text">
  The source of the adversarial input, which may be marked as "N/A" for security & safety datasets.
</ResponseField>
...
See [Security & Safety Threats](/concepts/product/dataset/security-threats) for a full mapping of threats to suggested metrics.