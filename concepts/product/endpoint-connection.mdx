...
<ResponseField name="Output Mapping" type="JSON" required>
  ...
  **Optional keys:**
  ...
  - `traces` - JSONPath expression to extract an array of [trace](/concepts/product/version/session/trace) objects (stored linked to the inference result). Each object should contain at least a `name` field and can include any of the [Trace properties](/concepts/product/version/session/trace#trace-properties): `type`, `description`, `inputData`, `outputData`, `error`, `latencyMs`, `metadata`, `startTime`, `endTime`
