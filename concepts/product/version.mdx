...
| `traces` | Extracts an array of [trace](/concepts/product/version/session/trace) objects and stores them linked to the inference result. Each object should follow the [Trace properties](/concepts/product/version/session/trace#trace-properties) schema (`name` is required). |
