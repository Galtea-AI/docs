...
| `{{ inference_result_id }}` | The id of this turn's [Trace](/concepts/product/version/session/trace), labeled "Galtea Trace ID" in the dashboard. The placeholder keeps its old name. Also sent as the `X-Galtea-Inference-Id` header for [span collection](/sdk/tutorials/tracing-agent-operations) |
