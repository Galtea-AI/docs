...
description: "Reference for the {{ input.field_name }} and {{ context.field_name }} template syntax used in endpoint connection input templates."
icon: "brackets-curly"
---
...
### Input with Language Information

If your test cases include a language field in their input schema, you can use it directly in the template:

```jinja2
{
  "model": "gpt-4",
  "messages": [
    {"role": "system", "content": "Always respond in {{ input.language }}."},
    {"role": "user", "content": "{{ input.user_message }}"}
  ]
}
```
