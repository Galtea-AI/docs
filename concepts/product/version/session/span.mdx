...
Galtea reads the type you sent and never the span name, so a step you called `search_tool` is only a **TOOL** if you typed it as one. A label that names a handoff or a transfer to an agent becomes **AGENT**. A label that ends in `tool` becomes **TOOL**. Everything else becomes **SPAN**, including a span you send with no type at all.
