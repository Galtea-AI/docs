...
<ResponseField name="Metadata" type="any" optional>
  ...
  Four keys are reserved for Galtea and removed from anything you send, without an error: `galtea.synthetic`, `galtea.span.original_name`, `galtea.span.input_from_source_rule` and `galtea.span.output_from_source_rule`. Galtea writes them itself to record what it changed or recovered. A fifth, `galtea.span.original_type`, is removed from a direct span write and kept on OpenTelemetry ingest, where a replayed export legitimately carries your original label in it.
</ResponseField>
