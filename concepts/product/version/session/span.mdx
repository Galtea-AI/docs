...
Spans are **always linked to an [inference result](/concepts/product/version/session/inference-result)**, enabling you to understand not just *what* your agent responded, but *how* it arrived at that response. Every span must belong to a specific inference result.
...
- Put the retrieved data in the span's **output**. This is how Galtea reads retrieval context: it is derived from your `RETRIEVER` spans. It is no longer a stored field on the inference result.
- The output has **no required shape**. It is stored as-is, so a plain string, a list of chunks, or any other structure all work.
- One RETRIEVER span per retrieval step is fine. If an inference result has several RETRIEVER spans, Galtea joins their outputs in chronological order (by start time).
...
<ResponseField name="Inference Result" type="InferenceResult" required>
  The inference result this span belongs to. Every span must be linked to an inference result.
</ResponseField>
...
<CardGroup cols={2}>
  ...
  <Card title="Inference Result" icon="message-bot" href="/concepts/product/version/session/inference-result">
    The inference results that spans are linked to.
  </Card>
</CardGroup>
