...
<ResponseField name="Output Files" type="AttachedFile[]" optional>
  ...
  On a run Galtea makes through an endpoint connection, the files come from the response instead: the connection's `output_files` key names them, and Galtea stores a copy of each, or links a file already in Galtea. See [Files in the Response](/concepts/product/endpoint-connection-configuration#files-in-the-response).
