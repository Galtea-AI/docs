...
- Put the retrieved data in the trace's **output**. This is the convention Galtea will use to read retrieval context from traces. That reader arrives in a later release. Today, retrieval context comes from the `retrieval_context` field on the inference result.
- The output has **no required shape**. It is stored as-is, so a plain string, a list of chunks, or any other structure all work.
- One RETRIEVER trace per retrieval step is fine. If an inference result has several RETRIEVER traces, Galtea will join their outputs in chronological order once the reader ships.
