...
<ResponseField name="Launched By" type="User" optional>
  The user who launched the session, read-only and set by the API. Null for a production session or one ingested via OpenTelemetry: neither has a launching user.
</ResponseField>
