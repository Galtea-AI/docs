...
- **Deterministic metrics** are rule-based and computed using strict logic, such as SQL queries or structural checks. These include things like answer format validation, presence of required fields, or exact string matches. Their results are consistent and reproducible. Common examples of deterministic checks include:
  ...
  *   Boolean condition checks (e.g., ensuring a specific condition evaluates to true or false as expected)
  ...
  Currently the deterministic metrics accessible directly from the platform are [BLEU](/concepts/metric/bleu), [ROUGE](/concepts/metric/rouge), [METEOR](/concepts/metric/meteor), [Text Similarity](/concepts/metric/text-similarity), [Text Match](/concepts/metric/text-match), [IOU](/concepts/metric/iou) and [Spatial Match](/concepts/metric/spatial-match), [URL Validation](/concepts/metric/url-validation) and [Tool Correctness](/concepts/metric/tool-correctness).
...
| **[Tool Correctness](/concepts/metric/tool-correctness)** | Deterministic | Compares the tools used by the agent against the expected tools to determine if the correct functions were selected. |
...
<ResponseField name="Evaluation Type" type="Enum" required>
  ...
  <Tabs>
    <Tab title="AI Evaluation">
      ...
      <Note>The SDK `source` value is `partial_prompt`.</Note>
    </Tab>
    <Tab title="Human Evaluation">
      ...
      **Use cases:**
      ...
      <Note>The SDK `source` value is `human_evaluation`.</Note>
    </Tab>
    <Tab title="Self-Hosted">
      ...
      <Note>The SDK `source` value is `self_hosted`.</Note>
    </Tab>
  </Tabs>

  <Accordion title="Legacy: Full Prompt (Deprecated)">
    <Warning>
      Full Prompt is deprecated in the dashboard and has been replaced by **AI Evaluation**. It is still accepted via the SDK and API for backward compatibility, but new metrics should use `source="partial_prompt"` (AI Evaluation) instead.
    </Warning>

    This method gives you maximum control. You provide a complete judge prompt template with placeholders (e.g., `{input}`, `{actual_output}`). Galtea will populate the template and use an LLM to evaluate the output based on your exact instructions.

    <Note>The SDK `source` value is `full_prompt`.</Note>
  </Accordion>
</ResponseField>

<ResponseField name="User Groups" type="List[String]" optional>
  [User groups](/concepts/user-group) that determine which human annotators can evaluate this metric. Only applicable for **Human Evaluation** metrics.
  - If user groups are linked, only users in those groups can annotate.
  - If no user groups are linked, any user in the organization can annotate.
...
<ResponseField name="Evaluation Parameters" type="List[Enum]" required>
  <Info>
    This must not be provided if the evaluation type is set to "Self Hosted".
  </Info>
  ...
  <table>
    ...
    <tbody>
      <tr>
        ...
        <td>Accuracy Testing, Security & Safety, and Behavior</td>
      </tr>
      <tr>
        <td>**actual_output**</td>
      ...
      <tr>
        ...
        <td>Accuracy Testing and Security & Safety</td>
      </tr>
      <tr>
        <td>**context**</td>
  ...
  <Warning>
    To ensure accurate evaluation results, only select parameters in the `Evaluation Parameters` list that you need for your evaluation. For **AI Evaluation**, these parameters are automatically prepended to your `judge_prompt`. For **Human Evaluation**, they determine which data fields are displayed to annotators.
  </Warning>
</ResponseField>

## Creating Custom Metrics via AI Evaluation
...
Custom Judge metrics use custom judge prompts to evaluate LLM outputs based on your specific requirements. AI Evaluation simplifies metric creation by focusing on the core evaluation logic.

### How AI Evaluation Works

AI Evaluation is the standard approach for LLM-as-a-judge metrics. You provide only the core **evaluation criteria** and **scoring rubrics**, and Galtea automatically constructs the final prompt by combining your criteria with the selected evaluation parameters.
...
- `traces` - Execution traces from the agent
- `expected_tools` - List of tools expected to be used by the agent
- `tools_used` - List of tools actually used by the agent (inferred from traces)
...
## Creating Custom Metrics via Full Prompt (Legacy)

<Warning>
  Full Prompt is deprecated in the dashboard and has been replaced by **AI Evaluation**. It is still accepted via the SDK and API for backward compatibility, but new metrics should use AI Evaluation (`source="partial_prompt"`) instead.
</Warning>

<Accordion title="Full Prompt Documentation">
...
You can use the `{conversation_turns}` placeholder to analyze the entire dialogue. The placeholder will be replaced with a formatted string representing the conversation.
...
<Info>
The `conversation_turns` parameter can only be used with the deprecated **Full Prompt** evaluation type. It provides the complete conversation history, allowing you to evaluate contextual consistency, knowledge retention, and other conversational qualities.
</Info>
