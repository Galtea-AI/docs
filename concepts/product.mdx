...
The SDK allows you to list existing products, retrieve specific products by their ID or name, and delete products. Product creation is done through the [Galtea dashboard](https://platform.galtea.ai/). See the [Product Service API documentation](/sdk/api/product/service) for more details.
...
%%{init: {"theme": "neutral"}}%%
flowchart TD
    P[Product] --> Spec[Specifications]
    P --> V[Version]
    P --> T[Test]
    P --> EC[Endpoint Connection]
    V -.->|uses| Mdl[Model]
    Spec -.->|links to| M[Metrics]
    Spec -.->|auto-derives| T
    T --> TC[Test Cases]
    V --> S[Session]
    TC -.->|linked to| S
    S --> IR[Inference Results]
    IR --> Tr[Traces]
    IR -->|per turn| E[Evaluations]
    S -->|per conversation| E
    M -.->|scores| E
```
