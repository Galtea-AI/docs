...
<ResponseField name="Closed Session Inference Creation Policy" type="Enum" optional>
  ...
  Possible values:
  - `Reopen And Reevaluate` (the default): keep the inference and open the session again. The session's monitor evaluations move to the `Outdated` status, so a [Monitor](/concepts/product/monitor) scores the session again once it closes for good. Only production sessions reopen; a development (test) session is dropped as with `Ignore`.
