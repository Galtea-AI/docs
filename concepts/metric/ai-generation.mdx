...
Once a metric already has a judge prompt, **Update with AI** revises it from one line of written feedback instead of you editing the prompt by hand. You give it the metric's current name, description, evaluation type, judge prompt, and evaluation parameters, plus a short instruction describing what to change (for example, "it should not penalise a short answer"). The AI returns a revised judge prompt, evaluation parameters, and description.

The endpoint **persists nothing** — you review the revision and decide whether to apply it.

In the dashboard, Update with AI is available only from the **New Revision** form (open a metric → **New Revision**), not from a blank or template create form. The dialog opens automatically once the form has loaded the source metric's current values, and a persistent **Update with AI** button at the top right reopens it at any time.

<Info>
  Update with AI requires the source metric to already have both a judge prompt and a description, and is not available for **Self-Hosted** metrics, which have no judge prompt.
</Info>

Applying a revision only fills in the form: the judge prompt, evaluation parameters, and description each get an "AI Generated" badge, but nothing is saved until you submit the **New Revision** form.

The endpoint is also available directly via the [REST API](/api-reference) and the CLI (`galtea metrics update-config`).
