...
<Warning>
  CAPABILITY and INABILITY specifications cannot be used for AI metric generation — only POLICY specifications can be
  linked to metrics. (CAPABILITY specifications do carry a test type, always `BEHAVIOR`, but that only makes them
  eligible for test generation, not metric generation.)
</Warning>
