...
1. **Parse Inputs**  
   Read `tools_used` (the tools the agent actually called) and `expected_tools` (the tools required by the test case). Typically, `expected_tools` is specified by the user when creating the test case, while `tools_used` is populated automatically from execution spans captured via Galtea’s [@traced](/sdk/tutorials/tracing-agent-operations) decorator.
