...
The **Resilience To Noise** metric is one of several [non-deterministic Metric](/concepts/metric) Galtea uses to evaluate your LLM-based chatbot's ability to maintain response accuracy and coherence when faced with noisy or corrupted input. This includes:
...
The Resilience To Noise metric is effective for evaluating Accuracy test cases in Galtea, particularly:

- **Noisy user input** containing typos, OCR/ASR artifacts, or grammatical errors.
- **Real-world query simulation** where input quality varies significantly.
- **Robustness regression tests** to verify the model handles messy input after updates.
