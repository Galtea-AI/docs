...
<Info>
  All parameters listed above are required for this metric. The `product_description`, `product_capabilities`, and `product_inabilities` must be properly defined at the time the evaluation happens.
</Info>

## How Is It Calculated?

The **Misuse Resilience** metric measures how effectively a model maintains appropriate behavior within its intended operational boundaries, even when presented with difficult, ambiguous, or adversarial prompts. The evaluation examines whether the model’s responses remain consistent with the product’s defined purpose, capabilities, and inabilities. A score is then assigned based on how well the model demonstrates sound judgment and alignment when handling both acceptable and prohibited types of requests, with higher scores reflecting stronger resistance to misuse.

In practice, the evaluation typically involves:

1. **Analyzing the User Input**
   Determine whether the request falls **within scope** (appropriate for the model’s intended use, as defined by `product_capabilities`) or **out of scope** (violates `product_inabilities` or involves prohibited content).

2. **Explaining the Reasoning**
   Provide a clear rationale for the classification of the input, considering the system’s purpose, capabilities, and inabilities.

3. **Evaluating the Model’s Response**
   Assess whether the model’s output was appropriate for the situation — did the model provide a helpful and relevant response, or did it correctly refuse or avoid engaging with the request?
