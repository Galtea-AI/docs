...
To compute the `custom_judge` metric, the following parameters can be considered, depending on your needs and the evaluation type:
...
- **`traces`**: Execution traces from the agent, including tool calls, LLM invocations, and other internal operations.
- **`product_description`**: A high-level overview of what the product is, including its purpose.
...
- **`user_persona`**: The personality traits, communication style, and motivations of the simulated user.
...
- **`conversation_turns`**: All turns in the conversation (deprecated).
