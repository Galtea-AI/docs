...
def rag_pipeline(query: str, trace_id: str) -> str:
...
# Create a trace to associate spans with
trace_for_rag = galtea.traces.create(
    session_id=session_start_span.id,
    input="What is the capital of France?",
)
if trace_for_rag is None:
    raise ValueError("trace_for_rag is None")

result = rag_pipeline("What is the capital of France?", trace_for_rag.id)
# @end start_span_rag_pipeline


# @start start_span_nested
def process_with_nested_spans(trace_id: str) -> dict:
    token = set_context(trace_id=trace_id)
...
# Create another trace for nested span example
trace_for_nested = galtea.traces.create(
    session_id=session_start_span.id,
    input="Process all items",
)
if trace_for_nested is None:
    raise ValueError("trace_for_nested is None")

nested_result = process_with_nested_spans(trace_for_nested.id)
