...
- **Homebrew (macOS & Linux):** `brew update && brew upgrade galtea`
- **APT (Debian/Ubuntu):** `sudo apt update && sudo apt upgrade galtea`
- **YUM/DNF (Fedora/RHEL):** `sudo dnf upgrade galtea`
- **PyPI (any OS):** `pip install --upgrade galtea-cli`

### Uninstall

- **Homebrew:** `brew uninstall galtea` (optionally `brew untap galtea-ai/tap` to drop the tap entirely)
- **APT (Debian/Ubuntu):** `sudo apt remove galtea`
- **YUM/DNF (Fedora/RHEL):** `sudo dnf remove galtea`
- **PyPI:** `pip uninstall galtea-cli`
