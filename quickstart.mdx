...
The Analytics page also shows an **evaluation scores over time** chart. It plots the average score per day across all evaluations that match your current filters. Use the toggle to switch between two views: **by metric** (one line per metric) and **by specification** (one line per specification, built from the metrics linked to each specification). The chart follows the same filters as the rest of the page, so changing the version, test, metric, language, time frame, or the development/production toggle updates it too. This aggregates every matching evaluation, including ones you triggered manually, so it is a product-wide trend, not one monitor's view.
