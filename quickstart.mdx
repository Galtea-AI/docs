...
test = galtea.tests.create(
    ...
    ground_truth_file_path="path/to/your/knowledge_base.pdf",
    language="english",
    max_test_cases=20,
)
```
...
```python
test_cases = galtea.test_cases.list(test_id=test.id)
print(f"Using test '{test.name}' with {len(test_cases)} scenarios.")
```
  </Tab>
</Tabs>
...
This is where you connect Galtea to your product. For each test case (or scenario), you run your system to produce an output and submit it to Galtea for scoring via [evaluations](/concepts/product/version/session/evaluation).
...
def your_product_function(input_prompt):
    return f"(your model output) {input_prompt}"
...
print(f"Submitted evaluations for version {YOUR_VERSION_ID} using test '{test.name}'.")
```
  </Tab>
...
# Placeholder for your actual product/model inference function
def your_product_function(input_prompt):
    # Example: refuse misuse requests
    if "ignore" in input_prompt.lower() or "password" in input_prompt.lower():
...
```
  ...
  <Tab title="Conversational">
```python
from galtea import Agent, AgentInput, AgentResponse
...
class MyAgent(Agent):
    def call(self, input_data: AgentInput) -> AgentResponse:
        ...
        model_output = f"(your model output) {user_message}"
        return AgentResponse(content=model_output)

for test_case in test_cases:
    # Create a session linked to the scenario
...
print(f"Submitted conversational evaluations for version {YOUR_VERSION_ID} using test '{test.name}'.")
```
  </Tab>
</Tabs>
...
Congratulations! You’ve completed your first evaluation with Galtea. Next, explore these concepts to tailor tests, metrics, and workflows to your product:
