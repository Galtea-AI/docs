...
On first sign-in, Galtea creates a new user account linked to the SAML identity. On later sign-ins, the same account is reused. Matching happens only inside the organization the sign-in targets: Galtea matches accounts by **email** and by **SAML NameID**, and on first sign-in the two get linked, so an existing account found by one gets the other attached. If the email belongs to an active account in a different organization, Galtea refuses the sign-in instead of matching, reusing, or changing that account. If the email and the SAML NameID already belong to two different accounts in the same organization, sign-in fails and the accounts must be reconciled manually.
...
| `saml_auth_failed` / `Invalid signature` | This code has two distinct causes. (1) The certificate is wrong, **or** the **Signed Response** / **Signed Assertions** toggles don't match what the IdP signs. (2) The asserted email belongs to an active user of a different organization, so Galtea refuses the sign-in on purpose. | For cause (1): re-download the IdP certificate and paste it again. If the certificate is correct, make **Signed Response** / **Signed Assertions** match exactly what your IdP signs (see [Configuration fields](#configuration-fields)); for example, disable **Signed Assertions** if the IdP signs only the response. For cause (2): an owner of the organization that holds the address must remove the account there, or the person must sign in with an address that organization does not hold. |
