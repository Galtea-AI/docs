...
<ResponseField name="max_turns" type="int">
  Number of conversation turns this simulation may add. Defaults to the Test Case's max turns if not provided. When the test case [starts from a stored conversation](/concepts/product/dataset/test-case#starting-a-scenario-from-a-conversation), the copied turns do not count against it: 4 stored turns with `max_turns=1` call your agent once.
</ResponseField>
...
<ResponseField name="agent_goes_first" type="bool">
  ...
  <Note>
    ...
    - It cannot be combined with a test case that [starts from a stored conversation](/concepts/product/dataset/test-case#starting-a-scenario-from-a-conversation). The opening turn is recorded before the stored conversation can be placed in front of it, so the simulation is refused.
  </Note>
</ResponseField>
