...
<ResponseField name="agent" type="AgentType" required>
  ...
  <Warning>
    An agent called from `simulate()` cannot answer with files. `output_file_paths` on the
    returned `AgentResponse` raises a `ValueError`, whether or not the response also carries
    text. A simulated conversation carries no files yet, on either side, so they would be
    dropped and the turn stored without them. Log a turn that answers with a document outside a
    simulation, with `traces.create(output_file_paths=...)`.
  </Warning>
</ResponseField>

<ResponseField name="max_turns" type="int">
  Number of conversation turns this simulation may add. Defaults to the Test Case's max turns if not provided. When the test case [starts from a predefined conversation](/concepts/product/dataset/test-case#starting-a-scenario-from-a-conversation), the copied turns do not count against it: 4 predefined turns with `max_turns=1` call your agent once.
</ResponseField>
...
<ResponseField name="agent_goes_first" type="bool">
  ...
  <Note>
    ...
    - It cannot be combined with a test case that [starts from a predefined conversation](/concepts/product/dataset/test-case#starting-a-scenario-from-a-conversation). The opening turn is recorded before the predefined conversation can be placed in front of it, so the simulation is refused.
  </Note>
</ResponseField>
