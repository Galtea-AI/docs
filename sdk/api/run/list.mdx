...
Returns a list of [Run](/concepts/product/run) objects.
...
<ResponseField name="search" type="string" optional>
  Free text that matches either the label of a run (case-insensitive, anywhere in the label) or its
  number. The number is accepted with or without a leading `#`, so both `42` and `#42` find run 42.
</ResponseField>
<Note>
