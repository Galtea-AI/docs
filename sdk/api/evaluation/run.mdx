...
<ResponseField name="agent" type="AgentType" optional>
  ...
  - An agent function (sync or async) with one of three signatures: `(str) -> str`, `(list[dict]) -> str`, or `(AgentInput) -> AgentResponse`. For the full list of fields available on `AgentInput`, see [AgentInput Fields](/sdk/api/simulator/simulate#agentinput-fields).
