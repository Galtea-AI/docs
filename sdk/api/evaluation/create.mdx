...
<ResponseField name="run_id" type="string" optional>
  The [Run](/concepts/product/run) these evaluations join. Defaults to the run of the enclosing `galtea.runs.start()` block, when there is one. An explicit value wins over the block.
</ResponseField>
