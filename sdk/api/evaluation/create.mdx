...
You can evaluate a specific inference result by providing its ID instead of a session ID. Pass [Specification](/concepts/product/specification) IDs so the API resolves their linked metrics automatically (recommended), or list metrics explicitly.
<Tabs>
  <Tab title="Using specification IDs">
```python
...
<Info>
  When using `CustomScoreEvaluationMetric`, your `measure()` method receives an `inference_results` parameter containing `InferenceResult` objects — all turns for session evaluations, or a single-element list for single inference result evaluations. This enables conversation-level custom scoring. See [Evaluate with Custom Metrics](/sdk/tutorials/evaluate-with-custom-metrics#multi-turn-custom-metrics) for examples.
</Info>
