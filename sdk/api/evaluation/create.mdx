...
<Info>
  When using `CustomScoreEvaluationMetric`, your `measure()` method receives an `inference_results` parameter containing `InferenceResult` objects — all turns for session evaluations, or a single-element list for single inference result evaluations. This enables conversation-level custom scoring. See [Evaluate with Custom Metrics](/sdk/tutorials/create-evaluation-with-custom-scores#multi-turn-custom-metrics) for examples.
</Info>
