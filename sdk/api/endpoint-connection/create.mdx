...
<ResponseField name="type" type="string" required>
  ...
  in newer API versions can be used; the API rejects invalid ones.
</ResponseField>
<ResponseField name="http_method" type="string" default="POST">
  HTTP method. Valid values: `GET`, `POST`, `PUT`, `PATCH`, `DELETE`. Defaults to `POST`. Other string
...
<ResponseField name="auth_type" type="string" default="BEARER">
  Authentication type. Valid values: `NONE`, `BEARER`, `API_KEY`, `BASIC`. Defaults to `BEARER`. Other
  string values are sent to the API unchanged, so values added in newer API versions can be used;
  the API rejects invalid ones.
</ResponseField>
...
<ResponseField name="input_template" type="string" optional>
  Input template with Jinja2 placeholders. Required for `CONVERSATION` type. Must name at least one input field, such as `{{ input.user_message }}`. Add `{{ input_files[0].url }}` or `{{ input_files[0].base64 }}` to send an attached file to your endpoint. A test case that attaches files is refused without one of them. See [Templates & Mapping](/concepts/product/endpoint-connection-configuration#input-template).
</ResponseField>
...
<ResponseField name="retry_backoff_strategy" type="string" optional>
  Backoff strategy. Valid values: `FIXED`, `EXPONENTIAL`, `LINEAR`. Defaults to `EXPONENTIAL`. Other
  string values are sent to the API unchanged, so values added in newer API versions can be used;
  the API rejects invalid ones.
</ResponseField>
...
<ResponseField name="acknowledged_insecure_http" type="bool" optional>
  Set to `true` to accept an `http://` URL. Required when `url` is not HTTPS, unless the deployment allows plain HTTP for every connection. See [HTTP Endpoints](/concepts/product/endpoint-connection#http-endpoints).
</ResponseField>
<ResponseField name="conversation_end_rules" type="list[ConversationEndRule]" optional>
  Replies that mean your agent ended the conversation. On a match, the session completes and is evaluated instead of failing, and the match is never retried. Each `ConversationEndRule` has a `status_code` (100–599) and an optional `body_match`, a `ConversationEndBodyMatch` with a JSONPath `path` and the expected value `equals`. A `2xx` rule needs a `body_match`. At most 20 rules, and only for `CONVERSATION` type. Defaults to no rules. For example, `ConversationEndRule(status_code=409, body_match=ConversationEndBodyMatch(path="$.error.code", equals="conversation_ended"))`. See [Conversation End Rules](/concepts/product/endpoint-connection-configuration#conversation-end-rules).
</ResponseField>
