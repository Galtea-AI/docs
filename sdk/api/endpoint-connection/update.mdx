...
<ResponseField name="input_template" type="string" optional>
  New input template with Jinja2 placeholders. Keep `{{ input_files[0].url }}` or `{{ input_files[0].base64 }}` in the template if your test cases attach files, because a run over those test cases is refused without it. See [Input Template](/concepts/product/endpoint-connection-configuration#input-template).
</ResponseField>
...
<ResponseField name="conversation_end_rules" type="list[ConversationEndRule]" optional>
  New list of replies that mean your agent ended the conversation. It replaces the stored list; pass an empty list to remove every rule. Each `ConversationEndRule` has a `status_code` (100–599) and an optional `body_match` (`ConversationEndBodyMatch` with `path` and `equals`). A `2xx` rule needs a `body_match`. At most 20 rules, and only for `CONVERSATION` type. See [Conversation End Rules](/concepts/product/endpoint-connection-configuration#conversation-end-rules).
</ResponseField>
...
<Note>
  ...
  `auth_token` and `password` are the exception: `None` or an empty string keeps the stored credential. To remove one, change `auth_type`, which clears the credentials the new type does not use.
</Note>
