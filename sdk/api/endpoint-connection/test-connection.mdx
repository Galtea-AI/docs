...
<ResponseField name="http_method" type="string" optional>
  HTTP method. Valid values: `GET`, `POST`, `PUT`, `PATCH`, `DELETE`. Other string values are sent
...
<ResponseField name="input_template" type="string" optional>
  ...
  <Note>
    The test renders a placeholder attachment for `input_files`. Its URL points at nothing your endpoint can download, so a failed download in this test is expected. See [Attached Files and Audio](/concepts/product/endpoint-connection-template-syntax#attached-files-and-audio).
  </Note>
</ResponseField>
<ResponseField name="auth_type" type="string" optional>
  Authentication type. Valid values: `NONE`, `BEARER`, `API_KEY`, `BASIC`. Other string values are
...
<ResponseField name="type" type="string" optional>
  ...
  used; the API rejects invalid ones.
</ResponseField>
<ResponseField name="timeout" type="int" optional>
  Request timeout in seconds.
</ResponseField>
<ResponseField name="acknowledged_insecure_http" type="bool" optional>
  Set to `true` to test an `http://` URL that is not saved yet. Required when `url` is not HTTPS, unless the deployment allows plain HTTP for every connection. See [HTTP Endpoints](/concepts/product/endpoint-connection#http-endpoints).
</ResponseField>
...
<Note>
  ...
  `auth_token` and `password` are the exception: `None` or an empty string keeps the stored credential. Changing `auth_type` drops the credentials the new type does not use, so the test matches what saving would produce.
</Note>
