...
<ResponseField name="messages" type="list[ConversationMessage]" required>
  ...
  - `input_files` (`list[InputFile]`): The files attached to a user message, ready for [`galtea.storage.download()`](/sdk/api/storage/download). Empty for a message that carries only text.
</ResponseField>
...
<ResponseField name="input_files" type="list[InputFile]">
