...
<ResponseField name="inference_result_id" type="Optional[str]">
  The inference result ID for this execution. Forward this to remote agents so they can attach spans to the correct session via `set_context(inference_result_id=...)`. See [Remote Agent Tracing](/sdk/tutorials/tracing-agent-operations#remote-agent-tracing).
</ResponseField>
