...
<ResponseField name="test_type" type="string | TestType" optional>
  ...
  - `ACCURACY` (or `QUALITY`): Evaluates whether the AI provides factually correct responses.
  - `SECURITY` (or `RED_TEAMING`): Tests for vulnerabilities, jailbreaks, and harmful outputs.
  - `BEHAVIOR` (or `SCENARIOS`): Simulates real user interactions to evaluate behavior in context.
</ResponseField>
