...
<Tip>
  To get the version of one agent state, use [Get or Create Version](/sdk/api/version/get-or-create) instead. It returns the version that holds your facts, and creates it only the first time. So a script that runs again, or two jobs that run at the same time, reuse one version.
</Tip>

## Returns
Returns a [Version](/concepts/product/version) object for the given parameters, or `None` if an error occurs. The returned object carries the `number` Galtea allocated, and the `name` the version is stored under.
...
<ResponseField name="name" type="string" optional>
  Your own label for the version. Leave it out and Galtea stores the version under its label, `v` followed by its [number](/concepts/product/version#how-a-version-is-identified), for example `v7`.

  A create with no name needs an API that allocates version numbers. A self-hosted deployment older than that still requires a name and refuses the create. Pass a name against such a deployment.

  <Warning>
    A name of `v` followed by digits is reserved for version labels. The check ignores upper and lower case, so `V7` follows the same rule as `v7`. Galtea accepts such a name only when it is the version's own label, so version 7 accepts `v7` and refuses `v3`. It always refuses `v01`, `v007` and `v0`, because no version is ever labelled that way. A name such as `v1.0` is outside the reserved shape and stays valid.
  </Warning>
</ResponseField>
...
<ResponseField name="model_id" type="string" optional>
  Reference to a [model](/concepts/model) configured in the Galtea Platform, used for cost tracking on this version's traces.
...
<ResponseField name="otel_input_source_rule" type="dict" optional>
  Recovers a turn's input from a non-LLM span, for products whose user message is not on the LLM span. An object with `spanName`, `attribute`, `path` and an optional `predicate`. The version value overrides the product value. See [Recover the turn input from a non-LLM span](/sdk/tutorials/monitor-real-user-traffic-with-opentelemetry#recover-the-turn-input-from-a-non-llm-span).
</ResponseField>
<ResponseField name="otel_output_source_rule" type="dict" optional>
  Recovers a turn's output from a non-LLM span, for templated or non-LLM replies. Same four parts as `otel_input_source_rule`. The version value overrides the product value. See [Recover replies that do not come from the LLM](/sdk/tutorials/monitor-real-user-traffic-with-opentelemetry#recover-replies-that-do-not-come-from-the-llm).
</ResponseField>
