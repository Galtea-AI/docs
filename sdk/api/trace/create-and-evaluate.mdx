...
<ResponseField name="output" type="string | object" optional>
  The generated output/response from the AI model. A dict is stored as sent, including any `content` array of file or audio parts; the judge reads only the text beside the parts.

  Required unless `output_file_paths` carries the whole answer. Give one of the two, or the call raises a `ValueError`.
</ResponseField>
<ResponseField name="output_file_paths" type="list[str]" optional>
  ...
  A custom scoring function still receives the text of the output, never the files.
...
<ResponseField name="run_id" type="string" optional>
  The [Run](/concepts/product/run) the evaluations join. Defaults to the run of the enclosing `galtea.runs.start()` block, when there is one. An explicit value wins over the block.
</ResponseField>
