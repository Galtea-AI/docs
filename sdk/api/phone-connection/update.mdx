...
<ResponseField name="voice" type="string" optional>
  New TTS voice for the simulated caller (Speechmatics, English-only). Supported values: `sarah` (en-GB female), `theo` (en-GB male), `megan` (en-US female), `jack` (en-US male). Pass `None` to clear it back to the default voice (`sarah`). An unsupported value is rejected with HTTP 400. Use [`list_voices()`](/sdk/api/phone-connection/list-voices) to retrieve the current catalog.
</ResponseField>
