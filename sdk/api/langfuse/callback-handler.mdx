...
<ResponseField name="inference_result_id" type="str" optional>
  Galtea inference result ID to link spans to. When provided, the handler automatically manages `set_context` / `clear_context` around LangChain callback lifecycles. All spans created during the invocation are stamped with this ID. Can also be set or changed later via `set_inference_result_id()`.
</ResponseField>
