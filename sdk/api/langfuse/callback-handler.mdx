...
description: "Drop-in replacement for Langfuse's langchain.CallbackHandler with Galtea span linking."
---
...
You can also pass `trace_id` directly in the constructor:
...
<ResponseField name="trace_id" type="str" optional>
  Galtea trace ID to link spans to. When provided, the handler automatically manages `set_context` / `clear_context` around LangChain callback lifecycles. All spans created during the invocation are stamped with this ID. Can also be set or changed later via `set_trace_id()`.
</ResponseField>
...
<ResponseField name="set_trace_id(trace_id)" type="method">
  Set or update the trace ID for subsequent invocations. Use this when reusing a single handler across multiple requests (singleton pattern). Pass `None` to disable Galtea correlation.
</ResponseField>
...
The handler automatically manages Galtea span context using depth tracking across LangChain callbacks:

- On the first `on_*_start` callback (root depth), `set_context(trace_id)` is called
