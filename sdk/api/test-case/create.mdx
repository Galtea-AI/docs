...
<ResponseField name="language" type="string" optional>
  Language for the test case. Accepts a [BCP-47 language tag](https://en.wikipedia.org/wiki/IETF_language_tag): a bare ISO 639-1 code (`"es"`) or a code with a region subtag (`"es-MX"`, `"en-GB"`). The region subtag is applied **only** when synthesizing speech for voice evaluations (the accent the simulated caller speaks with); text generation uses the base language only. A regional accent applies to any test type run over a phone connection (single-turn or multi-turn).
</ResponseField>
