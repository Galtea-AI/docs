...
<Note>
  The simulated caller's spoken language is taken from each test case's language, not the WebRTC connection, so one connection can serve test cases in different languages. See [WebRTC Connection](/concepts/product/webrtc-connection#how-the-callers-language-is-chosen).
</Note>
