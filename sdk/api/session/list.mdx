...
<ResponseField name="user_id" type="string | list[string]" optional>
  Filter by the ID of the user who launched the session. A production session has no launcher and never matches this filter. A session ingested from OpenTelemetry traces is production too, unless its spans name a test case; that one records the user of the API key that sent the traces, so it does match. This is an additional filter; it does not satisfy the scoping requirement below.
</ResponseField>
<Note>
  At least one of the scoping filters (`version_id`, `test_case_id`, `custom_id`, `dataset_id`, `id`) described above must be passed to the method. `status` and `user_id` are additional filters and do **not** satisfy this requirement on their own.
</Note>
