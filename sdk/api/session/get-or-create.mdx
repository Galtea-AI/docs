...
<ResponseField name="version_id" type="string" optional>
  ...
  <Note>When a new session is created, provide `version_id` or `product_id` (at least one). If both are given, `version_id` wins and `product_id` is ignored. Not used when an existing session is found.</Note>
</ResponseField>
<ResponseField name="product_id" type="string" optional>
  The ID of the product to associate with this session if it needs to be created, as an alternative to `version_id`. The API resolves the product's latest version, or creates a default one if the product has none yet.
