...
sidebarTitle: "Finish"
description: "Mark a session as finished (closed) so it becomes eligible for evaluation"
---

## Overview

The `finish()` method marks a session as finished when its conversation is over. Finishing sets the session status to `COMPLETED` and closes it.
