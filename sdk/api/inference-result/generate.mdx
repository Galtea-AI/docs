...
<ParamField path="agent" type="AgentType" required>
  ...
  - **Structured:** `(input_data: AgentInput) -> AgentResponse` — structured input/output. For the full list of fields available on `AgentInput`, see [AgentInput Fields](/sdk/api/simulator/simulate#agentinput-fields).
