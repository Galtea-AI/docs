...
The `generate()` method is the recommended way to execute your agent when you want automatic span collection, timing, and inference result creation. It handles the complete lifecycle:
...
| **Retrieval Context** | `AgentResponse.retrieval_context` | Context used (for RAG). Stored as a `RETRIEVER` span, not a readable field on the returned inference result. |
