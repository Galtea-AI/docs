...
| Error | Cause |
|-------|-------|
| `EntityNotFoundException` | The job does not exist, or belongs to a different organization (the API returns 404 in both cases to avoid leaking cross-tenant existence). Import it with `from galtea import EntityNotFoundException`. |
