...
- The launch that carried the job moves to `CANCELLED`. Its [Run](/concepts/product/run) follows only when a launch endpoint opened that Run and no other launch of it is still running. A Run you opened with `POST /runs` ends only when you close it, though the launch inside it is still cancelled.
...
| Error | Cause |
|-------|-------|
| `EntityNotFoundException` | The job does not exist, or belongs to a different organization (404). Import it with `from galtea import EntityNotFoundException`. |
| `JobAlreadyTerminalException` | The job has already reached `completed` or `failed` state and cannot be cancelled (409). Import it with `from galtea import JobAlreadyTerminalException`. |
