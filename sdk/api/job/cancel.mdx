...
- The [Run](/concepts/product/run) that carried the job moves to `CANCELLED`. Only a Run the platform opened carries a job, so a Run you opened with `POST /runs` is never reached this way; it still ends only when you close it.
