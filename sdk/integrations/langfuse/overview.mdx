...
If you already instrument your LLM app with [Langfuse](https://langfuse.com) — whether using `@observe` decorators or the LangChain `CallbackHandler` — you can send the same traces to Galtea for evaluation by swapping a single import. Under the hood, one span per function call flows to both Langfuse and Galtea simultaneously.
...
<CardGroup cols={2}>
  ...
  <Card title="Selective Export" icon="filter">
    Galtea only exports spans when you explicitly link a `trace_id`. Otherwise, it does nothing.
  </Card>
</CardGroup>
...
Decorate your agent functions with `@observe`, exactly as you would with Langfuse. Nested decorators create a parent-child span hierarchy automatically.
...
When `my_agent("Hello", trace_id="trace_abc123")` runs, Galtea receives a 3-level span hierarchy:
...
Use the `as_type` parameter on `@observe` to set the [observation type](https://langfuse.com/docs/observability/features/observation-types). Each type maps to a Galtea span type automatically:
...
| Langfuse `as_type` | Galtea span type | Description |
|---|---|---|
...
<Info>
  Unknown Langfuse observation types are automatically mapped to `SPAN` in Galtea. Your spans are never dropped.
</Info>
...
Your handler initialization stays the same — create it once at app startup and pass it to any LangChain `.invoke()`, `.batch()`, or `.stream()` call. To link spans to Galtea, call `set_trace_id` before each invocation:
...
The handler automatically manages `set_context` / `clear_context` around LangChain callback lifecycles — no context managers or manual cleanup needed. You can also pass `trace_id` directly in the constructor if you prefer to create a handler per request.

<Note>
  The `CallbackHandler` requires `langchain` to be installed. Install it with: `pip install langchain`
</Note>

All three APIs can be mixed freely — for example, an `@observe`-decorated function can pass a `CallbackHandler` to a LangChain chain inside it, and the parent-child span hierarchy is preserved.

## Linking Spans to a Trace

Galtea **only** exports span data when a `trace_id` is explicitly linked. Without it, Galtea does nothing — no data is sent, no spans are modified. There are three ways to link spans:

### Using SDK methods (recommended)

When using `generate()` or `simulate()`, the SDK manages `trace_id` automatically — zero extra code:
...
### Passing `trace_id` as a kwarg

Pass `trace_id` to the outermost `@observe`-decorated function. The wrapper manages the span context automatically:
...
The `trace_id` kwarg is consumed by the wrapper — it does not reach your function's parameters.
...
Both Galtea and Langfuse use OpenTelemetry internally. When both are initialized, they share the same tracing infrastructure — each span created by `@observe` flows to both Langfuse cloud and the Galtea API. The Galtea SDK exporter only sends spans that have a `trace_id` linked (it posts them to the Galtea REST `/traces` endpoint, which ingests spans); everything else is skipped client-side. Langfuse observation attributes (type, input, output, metadata) are automatically mapped to their Galtea equivalents. (Spans exported directly to the Galtea public OTel endpoint follow different rules; see [Send OpenTelemetry Traces to Galtea](/sdk/tutorials/send-opentelemetry-traces-to-galtea).)
