...
<Tabs>
  <Tab title="@observe (decorator)">
    ...
    `@observe` works on `async def` functions too. The wrapper detects the coroutine and awaits it before clearing context, so spans are correctly stamped with `trace_id`.
...
<Note>
  When using Galtea's SDK methods (`generate()`, `simulate()`), this step is not needed — the SDK manages `trace_id` internally.
</Note>
