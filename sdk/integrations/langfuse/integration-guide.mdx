...
description: "What changes when you add Galtea evaluation to your Langfuse-instrumented app"
...
<Note>
  Galtea uses inference results as the unit of evaluation — each one represents a single input/output pair that can be scored by metrics. By linking spans to an inference result, Galtea knows which execution spans belong to which response, enabling span-aware evaluations and hence full visibility into how your agent arrived at each answer.
</Note>
...
<Tabs>
  <Tab title="@observe (decorator)">
    ...
    `@observe` works on `async def` functions too. The wrapper detects the coroutine and awaits it before clearing context, so spans are correctly stamped with `inference_result_id`.
  ...
  <Tab title="start_as_current_observation (context manager)">
    ...
    <Note>
      `start_as_current_observation` is a synchronous context manager. Inside `async def` code, use plain `with` (not `async with`); `async with` raises `TypeError`. For async agents, prefer the `@observe` decorator.
    </Note>

    <Tip>
      Set `as_type` to the step's real type. If you log a custom retriever, tool, or agent step as a plain span, set `as_type="retriever"` (or the matching type). Galtea then imports it as that span type instead of a generic `SPAN`. Galtea reads the exported observation's `type` field, which Langfuse sets from `as_type`. It never guesses the type from the span name.
    </Tip>
  </Tab>
  <Tab title="CallbackHandler (LangChain)">
...
<Note>
  When using Galtea's SDK methods (`generate()`, `simulate()`), this step is not needed — the SDK manages `inference_result_id` internally.
</Note>
