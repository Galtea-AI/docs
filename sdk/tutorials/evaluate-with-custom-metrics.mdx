...
When using `CustomScoreEvaluationMetric`, your `measure()` method always receives a `traces` parameter with `Trace` objects. For session evaluations (`session_id=...`) this includes all turns; for single trace evaluations (`trace_id=...`) it contains one item. This enables conversation-level scoring such as consistency checks, cross-turn analysis, or aggregated metrics.
...
<Info>
  The `traces` parameter is always a list ordered chronologically. For single-turn evaluations (via `trace_id`), it contains one item. For session evaluations, it contains all turns. See [Evaluating Conversations](/sdk/tutorials/evaluating-conversations) for the full session-based workflow.
</Info>
