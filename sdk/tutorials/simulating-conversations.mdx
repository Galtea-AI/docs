...
Galtea's [Conversation Simulator](/concepts/product/dataset/test-case/conversation-simulator) allows you to test your AI products — chatbots, assistants, and agents — by simulating realistic multi-turn user interactions. This guide walks you through integrating your agent and running simulations using [Behavior Datasets](/concepts/product/dataset/behavior-datasets).

<Tip>
  **Using the specification-driven workflow?** If you've defined [Specifications](/concepts/product/specification) with linked metrics and datasets, the evaluation pipeline handles simulation automatically — no manual session creation needed. See [Evaluations from Specs](/sdk/tutorials/specification-driven-evaluations).
</Tip>
...
<Steps>
  ...
  <Step title="Create a Dataset and Sessions">
    Upload your scenario CSV to create a dataset, or use AI-generated datasets from the dashboard. The platform generates a session for each test case.
  </Step>
  ...
  <Step title="Evaluate the Results">
    After simulation, trigger evaluations via `evaluations.create()` or review results in the dashboard's **Analytics** section.
  </Step>
</Steps>

### Step-by-Step Guide

#### 1. Create a Dataset and Sessions
...
Once generation completes, you'll see the resulting test cases in your dashboard. For the CSV upload format, see [Behavior Datasets](/concepts/product/dataset/behavior-datasets).
...
<Info>
  You can optionally use the `@traced` decorator to capture internal operations during simulation. Spans are automatically collected and saved per turn.
...
For Retrieval-Augmented Generation (RAG) agents, you can return the context that was retrieved and used to generate the response. This context will be logged with the trace, enabling evaluations with metrics like `Faithfulness` and `Contextual Relevancy`.
