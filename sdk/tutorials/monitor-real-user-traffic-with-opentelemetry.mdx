...
<Note>
  ...
  - **Closed sessions reject new turns.** A session is closed once it reaches `COMPLETED` or `FAILED`, either by an explicit finish or by the auto-close sweep (see [Session lifecycle](/concepts/product/version/session#session-lifecycle)). Late spans for turns it already has are still accepted. Spans for a new turn are rejected and appear in the export response's rejected-span count; nothing is stored. To start new turns, begin a new conversation with a fresh `galtea.session.custom_id` (or a new Temporal workflow).
