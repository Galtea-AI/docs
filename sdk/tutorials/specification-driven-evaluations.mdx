...
<Tip>
  **Setup for evaluation** does Step 2 and Step 3 at once. Saving a Capability or Policy by hand lands on its page, which opens the dialog on its own when there is something to generate; it stays available afterwards on that specification's own page, in place of Run Evaluation, and from its row menu in the Specifications table. Saving an Inability never opens it, since an inability needs no dataset and no metric. You set how many test cases to generate, optionally pick a language instead of leaving it on Automatic, attach the knowledge base file if the specification is `ACCURACY`, and confirm. It creates whatever that specification is still missing — the dataset, its metrics, or both — then tells you what it did. The dataset's test cases keep generating in the background. If it generated metrics, click **Review** to open the review page and accept or edit them there; closing the dialog instead discards them, and you would have to generate them again.
</Tip>
