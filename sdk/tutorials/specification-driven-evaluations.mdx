...
<Info>
  For running evaluations using specifications, see [Specification-Based Evaluation](/sdk/tutorials/run-dataset-based-evaluations#specification-based-evaluation).
</Info>
...
<Note>
  AI dataset generation is available for **Policy** specifications with a `SECURITY` or `BEHAVIOR` dataset type, and for every **Capability** specification (always `BEHAVIOR`). The system uses the specification's description as context — for Security datasets it becomes the `custom_variant_description`, and for Behavior datasets it shapes the scenario generation.
</Note>
...
<CardGroup cols={2}>
  <Card title="Run Specification-Based Evaluations" icon="clipboard-check" href="/sdk/tutorials/run-dataset-based-evaluations#specification-based-evaluation">
