...
<Info>
  Each `InferenceResult` object provides `actual_input`, `actual_output`, `latency`, `index`, and other fields. Retrieval context is not a field on the object; it is derived from the turn's `RETRIEVER` spans. See the [custom metrics tutorial](/sdk/tutorials/evaluate-with-custom-metrics) for more on custom metrics.
</Info>
