...
When you use `CustomScoreEvaluationMetric`, your `measure()` method always receives a `traces` parameter containing `Trace` objects. For session evaluations this includes all turns; for single trace evaluations it contains one item. This enables conversation-level scoring (e.g., consistency checks, cross-turn analysis).
...
<Info>
  Each `Trace` object provides `actual_input`, `actual_output`, `latency`, `index`, and other fields. Retrieval context is not a field on the object; it is derived from the turn's `RETRIEVER` spans. See the [custom metrics tutorial](/sdk/tutorials/evaluate-with-custom-metrics) for more on custom metrics.
</Info>
