...
When running evaluations via Direct Inference, you can collect [traces](/concepts/product/version/session/trace) from your endpoint handler by passing the `{{ inference_result_id }}` placeholder in your input template. This lets your endpoint know which inference result the call belongs to, so it can link traces back to Galtea.
