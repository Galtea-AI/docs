---
title: 'Create a Custom Test'
description: "Learn how to create and upload custom tests using the SDK"
icon: "clipboard-list"
---

While Galtea can automatically generate tests from a knowledge file, you can also create custom [Tests](/concepts/product/test) by providing your own set of [Test Cases](/concepts/product/test/case).

## Creating Tests with the SDK

<Info>
  Tests can be generated automatically by Galtea providing a knowledge file [via the SDK](/sdk/api/test/create) or the dashboard.
</Info>

The Galtea SDK provides methods to create [Accuracy Tests](/concepts/product/test/quality-tests), [Security & Safety Tests](/concepts/product/test/red-teaming-tests), and [Behavior Tests](/concepts/product/test/scenario-tests).


<Tabs>
  <Tab title="Upload Existing Test File">
    If you have a prepared test file in CSV format, you can upload it directly.

```python
# Upload a pre-existing test file to the Galtea Platform
test = galtea.tests.create(
    name="financial-qa-test-" + run_identifier,
    type="ACCURACY",
    product_id=product_id,
    test_file_path="path/to/accuracy_test.csv",
)
```
    
    <Warning>
      The test file must follow the structure specified for:
      - [Accuracy Tests](/concepts/product/test/quality-tests#example-accuracy-tests-and-file-format): Standard format with `input`, `expected_output`, etc.
      - [Security & Safety Tests](/concepts/product/test/red-teaming-tests#example-security-safety-tests-and-file-format): Standard format with adversarial inputs
      - [Behavior Tests](/sdk/tutorials/simulating-conversations): Conversation simulator format with `goal`, `user_persona`, etc.
      
      If the file is not correctly formatted, test cases will not be created automatically, but you can still [add them manually](/sdk/api/test-case/create).
    </Warning>
    
    <Info>
      **Behavior Tests**: Use `type="BEHAVIOR"` to create conversation simulation tests that enable multi-turn dialogue evaluation with synthetic users. See the [Conversation Simulator Tutorial](/sdk/tutorials/simulating-conversations) for complete examples.
    </Info>
  </Tab>

  <Tab title="Generate Test from Knowledge Base">
    You can provide a knowledge base file (e.g., PDF, TXT) and have Galtea generate the test cases for you. This is a powerful feature for creating comprehensive tests with minimal effort.

```python
# Generate a test from a knowledge base file (PDF, TXT, etc.)
generated_test = galtea.tests.create(
    name="generated-financial-qa-test-" + run_identifier,
    type="ACCURACY",
    product_id=product_id,
    ground_truth_file_path="path/to/knowledge.md",
    language="english",
    max_test_cases=50,  # Limit the number of generated test cases
)
```
  </Tab>
</Tabs>

## Next Steps

<CardGroup cols={2}>
  <Card title="Run Test-Based Evaluations" icon="badge-check" href="/sdk/tutorials/create-evaluation">
    Run your test against a specific product version.
  </Card>
  <Card title="API Reference" icon="code" href="/sdk/api/test/service">
    View the complete Test Service API reference.
  </Card>
</CardGroup>