...
description: "Configure an OpenTelemetry exporter or Collector to reach Galtea's public OTel endpoint: endpoint, authentication, and how spans map to Span records."
icon: "tower-cell"
---
...
- **Create production sessions from real traffic.** Uncorrelated spans become production [Sessions](/concepts/product/version/session) that [Monitors](/concepts/product/monitor) evaluate automatically. Spans that name a test case become a non-production session instead, which no Monitor scores. See [Monitor Real User Traffic via OpenTelemetry](/sdk/tutorials/monitor-real-user-traffic-with-opentelemetry).
...
4. **A configured span-source rule** — `otelInputSourceRule` or `otelOutputSourceRule` on the Product or Version, which reads the field out of a named non-LLM span. See [Recover the turn input from a non-LLM span](/sdk/tutorials/monitor-real-user-traffic-with-opentelemetry#recover-the-turn-input-from-a-non-llm-span).

A **turn's** input and output follow a different order, because a turn has many spans: a configured source rule decides the field, else the turn's single root span, else the earliest input and the latest output. A configured rule that fires on no span of the turn leaves that field empty; it never falls back. See [Monitor real user traffic](/sdk/tutorials/monitor-real-user-traffic-with-opentelemetry#what-you-get).
...
<Tabs>
  ...
  <Tab title="OTel GenAI conventions">
    ...
    `gen_ai.usage.input_tokens` and `gen_ai.usage.output_tokens` give Galtea the call's token counts. See [Token usage and cost](#token-usage-and-cost) below.
...
Galtea reads the token counts your spans report and adds them up per turn. Send them with the OpenTelemetry GenAI keys:

| Attribute Key | Type | Description |
|---|---|---|
| `gen_ai.usage.input_tokens` | `int` | Input (prompt) tokens the call consumed. |
| `gen_ai.usage.output_tokens` | `int` | Output (completion) tokens the call produced. |

A turn's `inputTokens`, `outputTokens` and `tokens` are the sum over every span of that turn, so a turn that makes three LLM calls reports the three calls added together. A span that reports no usage adds nothing. Send no usage at all and the turn keeps what Galtea already had: nothing when no Model is linked to the Version, or the estimate Galtea derived from the turn text when one is.

Galtea also recognizes the usage a managed platform reports in its own format. MLflow Tracing's `mlflow.chat.tokenUsage` works with no Galtea attribute. When a span carries both, the `gen_ai.usage.*` keys win, field by field.

**Cost** is derived from those counts and the per-token price of the [Model](/concepts/model) linked to the Version. Link a Model with a known price and the turn's `cost` is filled. With no Model, or with no price for it, `cost` stays empty rather than zero.

A reported count replaces one Galtea estimated earlier: your own figure is the accurate one. A cost you reported yourself is kept, and the new counts do not replace it: Galtea recomputes only a cost it derived from the counts and the Model price, never a different figure you set. Spans of a turn may arrive in several export batches, and each batch raises the turn's total instead of replacing it. Re-sending the same spans changes nothing.
...
## Attributes that route a conversation

The keys above fill a Span record. Three more keys decide which session Galtea assembles the spans into. They apply to uncorrelated spans only, so a span already linked to a Trace ignores them.

| Attribute Key | Set On | Description |
|---|---|---|
| `galtea.session.custom_id` | Span | Your own conversation ID. Spans sharing it become one session. Galtea also reads the standard `gen_ai.conversation.id` and `session.id` keys when this one is absent. |
| `galtea.version.id` | Resource | The [Version](/concepts/product/version) the session is created under. Required: without it, uncorrelated spans are rejected. |
| `galtea.test_case.id` | Resource or span | The [Test Case](/concepts/product/dataset/test-case) the conversation exercises. Galtea creates the session as non-production and links it to that test case, so no [Monitor](/concepts/product/monitor) scores it. A span value marks one conversation and wins over the resource value. Omit the key to get a production session. |

Galtea rejects one conversation, while the rest of the payload still ingests, when the test case does not exist, when it belongs to another product than the version, when two spans of one conversation name different test case IDs or two resources do, or when the conversation key already holds another session on that version. Those spans are counted in the export response's `partialSuccess.rejectedSpans`. The response's `partialSuccess.errorMessage` names the reason that applied, so your collector log says what to change.

[Monitor Real User Traffic via OpenTelemetry](/sdk/tutorials/monitor-real-user-traffic-with-opentelemetry) walks through the full setup.
