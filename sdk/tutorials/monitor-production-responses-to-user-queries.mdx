...
When the user's conversation ends, finish the session with [`finish()`](/sdk/api/session/finish). This closes the session and makes it ready for evaluation.
...
Finishing matters most when you use a [Monitor](/concepts/product/monitor) to score production sessions automatically: a monitor scores only closed sessions. An explicit finish makes the session eligible on the next scan, instead of waiting for the product's inactivity auto-close window (30 minutes by default). Call `finish()` only once the conversation is truly over. A turn that arrives later follows the product's closed-session policy: by default the session opens again and the monitor scores it again, but the policy can also drop or reject the turn.
</Step>
</Steps>
...
A production session does not stay open forever. Galtea's auto-close sweep closes a session once it has been quiet for the product's **auto-close inactivity window** (default 30 minutes), and an explicit finish closes it at once. A [session](/concepts/product/version/session#session-lifecycle) is then closed (`COMPLETED` or `FAILED`).
...
- `REOPEN_AND_REEVALUATE` (the default): the turn is **stored** and the session opens again with status `PENDING`. Its existing monitor evaluations move to the `OUTDATED` status, and the monitor scores the session again once it closes. Nothing is lost. A development (test) session reopens the same way; it has no monitor evaluations, so only the turn is stored and the status changes.
- `IGNORE`: the log call returns success, but the turn is **not stored**. The late reply is silently dropped.
- `REJECT`: the log call **fails with an error**. Your code must handle it (for example, start a new session for the reply).

<Note>Each re-score spends evaluation credits again. The outdated evaluation keeps the credits it already used, so a session that reopens many times costs more and uses more of the monitor's monthly credit cap.</Note>

The default keeps late replies. If you set the policy to `IGNORE` or `REJECT`, a late reply is lost or raises an error. Two settings help in that case. Both are set per product from the API or the dashboard (the SDK reads these fields but does not set them):
...
- **A revision excluded from analytics leaves the series.** If you [exclude a metric revision from analytics](/concepts/metric#revisions-and-analytics), its results drop out of that family's score series and headline, the same way they drop out of the Analytics page. Restoring the revision brings them back.
...
- **A human review is the score Galtea reports.** When a reviewer scores an evaluation, every series and headline uses the reviewer's score, not the judge's. That evaluation counts even when the judge's own run failed, was skipped, or never finished.
...
- **Markers flag context changes.** `markers.metricRevisions` marks when a bound metric family was edited, and `markers.samplingChanges` marks when the sampling rate changed. Both can explain a shift in the score series that isn't a real quality change. An evaluation the judge never timestamped is dated by the time it ran, so a review never moves a marker.
