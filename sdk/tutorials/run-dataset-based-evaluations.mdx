...
<Info>
    A [session](/concepts/product/version/session) is automatically created behind the scenes to link this `version_id` and `dataset_id` with the provided [inference result](/concepts/product/version/session/inference-result) (the `actual_output` and the [Test Case](/concepts/product/dataset/test-case)'s input).
</Info>
