...
<Note>
  Capability specifications always get the `BEHAVIOR` test type, assigned automatically — you never choose it. This means every capability is eligible for AI test generation, the same as a Behavior policy.
</Note>
...
Describe rules the product **must follow** — refusal rules, mandatory disclaimers, interaction patterns, and behavioral guardrails. Policies can drive either Security or Behavior test generation, depending on their test type (Capabilities, by contrast, always drive Behavior test generation).
