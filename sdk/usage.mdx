...
Describe the agent state you test as [facts](/concepts/product/version#version-facts). Galtea returns the version that holds those facts, and creates it the first time. So you can run the same script again and it reuses the version.
...
```
<Info>
    More information can be found in [Get or Create Version](/sdk/api/version/get-or-create). To set other fields, such as a system prompt or an endpoint connection, see [Create Version](/sdk/api/version/create).
</Info>

### Creating a Dataset

With a [Dataset](/concepts/product/dataset) you define the [test cases](/concepts/product/dataset/test-case) to evaluate your [product](/concepts/product).
You can create [accuracy](/concepts/product/dataset/accuracy-datasets) or [security](/concepts/product/dataset/security-datasets) datasets depending on your evaluation needs.
...
<Info>
    More information about creating a dataset can be found [here](/sdk/api/dataset/create).
</Info>
...
<Info>
    More information about creating a metric can be found [here](/sdk/api/metric/create).
</Info>

### Launching Evaluations

[Evaluations](/concepts/product/version/session/evaluation) assess the performance of a [product](/concepts/product) [version](/concepts/product/version) against [test cases](/concepts/product/dataset/test-case) using specific [metrics](/concepts/metric). They are now directly linked to [sessions](/concepts/product/version/session) that contain all traces.

The example below loops over the test cases by hand, because it attaches a custom score object and a `retrieval_context` per test case, which only the manual loop can do.
...
<Tip>
  If you only need standard metrics, skip the loop: `galtea.evaluations.run()` resolves your [Specifications](/concepts/product/specification), their datasets and metrics, runs your agent on every test case, and submits the results in one call. See [Specification-Driven Evaluations](/sdk/tutorials/specification-driven-evaluations).
</Tip>
...
When listing resources that may contain many items (products, datasets, sessions, evaluations, etc.), the Galtea SDK uses pagination with a **default limit of 10,000 items** per request.
